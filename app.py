...
from db import buscar_codigo, init_db
...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Escáner Cazafantasmas AWS",
    page_icon="👻",
    layout="centered",
...
st.markdown("""
    <style>
        ...
        /* --- Optimizaciones para Móvil en Vertical (Portrait) --- */
        @media (max-width: 600px) {
            .main-header h2 {
                font-size: 1.35rem;
            }
            .result-card-found, .result-card-notfound {
                padding: 16px;
                border-radius: 12px;
            }
            .detail-row {
            ...
            /* Forzar que los radio buttons se apilen en vertical en pantallas pequeñas */
            div.row-widget.stRadio > div[role="radiogroup"] {
                flex-direction: column !important;
                gap: 8px !important;
            }
            /* Ajustar el padding principal de la app en móvil para aprovechar el espacio */
            .block-container {
...
def emitir_sonido(tipo: str):
    """Genera tonos de sonido directamente en el navegador del usuario."""
    import time
    ts = time.time()
    if tipo == "success":
        js_code = f"""
...
st.markdown("""
    <div class="main-header">
        <h2>👻 Escáner Cazafantasmas AWS</h2>
        <p>Lectura de código de barras + Verificación instantánea en PostgreSQL</p>
    </div>
""", unsafe_allow_html=True)
...
# Modo 1: Escáner Continuo en Tiempo Real (HTML5-QRCode)
if modo == "⚡ Escáner Continuo en Vivo":
    st.caption("📷 Apunta la cámara del móvil al código de barras (Detección automática en milisegundos)")
    ...
    <body>
        ...
        <script>
            function onScanSuccess(decodedText, decodedResult) {
                // Enviar el código escaneado de regreso a Streamlit mediante la URL hash o almacenamiento
    ...
    # Renderizamos el componente HTML5
    scanned_value = components.html(html_scanner, height=330)
    
    # Campo secundario para sincronizar el valor del escáner en vivo
    codigo_detectado = st.text_input("Código capturado en vivo:", key="live_input", placeholder="Escaneando cámara...")

# Modo 2: Foto con Cámara Estándar (zxing-cpp)
    ...
    if foto:
        try:
            ...
            if resultados:
                ...
                st.warning("No se identificó ningún código de barras en la foto. Intenta enfocar más cerca.")
        except Exception as e:
            st.error(f"Error procesando imagen: {e}")

# Modo 3: Entrada Manual o Escáner USB
elif modo == "⌨️ Entrada Manual / Escáner USB":
    codigo_detectado = st.text_input("Ingresa o escanea el código aquí:", key="manual_input")

# ---------------------------------------------------------
# Lógica de Búsqueda y Presentación de Resultados
# ---------------------------------------------------------
if codigo_detectado and codigo_detectado.strip() != "":
    codigo_limpio = codigo_detectado.strip()
    
    # Evitamos re-procesar el mismo código consecutivamente en bucle
    if st.session_state.ultimo_codigo_procesado != codigo_limpio:
        st.session_state.ultimo_codigo_procesado = codigo_limpio
        
        # Consultar la Base de Datos PostgreSQL de AWS RDS
        res, origen = buscar_codigo(codigo_limpio)
        st.session_state.resultado_actual = (res, origen, codigo_limpio)
...
if st.session_state.resultado_actual:
    res, origen, codigo_buscado = st.session_state.resultado_actual

    if res:
        # SONIDO POSITIVO (BEEP ÉXITO)
        ...
        st.markdown(f"""
            <div class="result-card-found">{ya_encontrado_html}
                ...
                <h3 style="margin: 0 0 10px 0; font-size: 1.3rem;">{res.get('desc_master_box', 'Producto')}</h3>
...
if st.session_state.historial:
    st.markdown("---")
    st.subheader("📋 Historial de Escaneos (Sesión Actual)")
    
    for item in st.session_state.historial[:8]: # Mostrar los últimos 8
        color_indicador = "🟢" if item["encontrado"] else "🔴"
        
        # Recuperamos datos, con soporte para historiales viejos en la misma sesión
